...
// EJECUTAR: ./ej2 <host> <puerto> <comando>
// ejemplo: ./ej2 :: 8080 t
...
int main(int argc, char *argv[]) {
    if (argc!=4) {
        fprintf(stderr, "Usage: %s <host> <puerto> <comando>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    len=strlen(argv[3])+1;
    if (write(sfd, argv[3], len)==-1) { perror("write()"); exit(EXIT_FAILURE);}
    
    if (read(sfd, buf, BUF_SIZE)==-1) { perror("read"); exit(EXIT_FAILURE); }
    printf("%s", buf);
