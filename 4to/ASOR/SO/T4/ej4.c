...
// TERMINA: cuando el cliente envia 'q' o con SIGINT (Ctrl + C) 
...
int main(int argc, char** argv){
    ...
    int max_fd=sfd+1, sel_fd;
    fd_set readfds;
    while(1){              
        FD_ZERO(&readfds);
        FD_SET(STDIN_FILENO, &readfds);
        FD_SET(sfd, &readfds);       
        sel_fd=select(max_fd, &readfds, NULL, NULL, NULL); // no hay timeout porque un fd=0 (STDIN)
        if(sel_fd==-1){ perror("select()"); exit(EXIT_FAILURE); }
        
        
        if(FD_ISSET(STDIN_FILENO, &readfds)){ // Terminal            
            bytes_read=read(STDIN_FILENO, query, BUF_SIZE);
            if(bytes_read<0) continue; /* Ignore failed request */
            query[bytes_read-1]='\0';
            printf("Terminal\tEnvia: %s\n", query);
        }
        else { // UDP
            salen=sizeof(sa);
